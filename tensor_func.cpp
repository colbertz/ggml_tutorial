...
void tensor_func_1d(){
    ...
    struct ggml_tensor * three_x_squared = ggml_mul(ctx, ggml_new_f32(ctx, 3.0f), x_squared); // 3 * x^2
    struct ggml_tensor * four_x = ggml_mul(ctx, ggml_new_f32(ctx, 4.0f), x); // 4 * x
    struct ggml_tensor * result = ggml_add(ctx, ggml_add(ctx, three_x_squared, four_x), ggml_new_f32(ctx, 5.0f)); // 3 * x^2 + 4 * x + 5
...
void tensor_func_2d(){
    ...
    a_data[0] = 5.0f; a_data[1] = 9.0f; a_data[2] = 1.0f;
    a_data[3] = 2.0f; a_data[4] = 3.0f; a_data[5] = 4.0f;
    a_data[6] = 7.0f; a_data[7] = 8.0f; a_data[8] = 6.0f;
...
void tensor_func_jttl(){
    ...
    a_inv_data[0] = 2.0f;   a_inv_data[1] = -0.5f;
    a_inv_data[2] = -1.0f;  a_inv_data[3] = 0.5f;
